...
<div className="max-w-7xl mx-auto">
  <div className="grid lg:grid-cols-2 gap-12 items-center">
    ...
    <motion.div initial={{opacity: 0, y: 20}} animate={{opacity:1 , y:0}} transition={{duration: 0.8}} className="text-center lg:text-left">
      <h1 className="text-4xl md:text-6xl lg:text-7xl font-serif font-bold text-foreground leading-tight mb-6">Air Quality for <span className="text-primary italic">Realtime</span></h1>
      <p className="text-xl md:text-2xl text-muted-foreground mb-8 max-w-2xl"> The airquality near me tell Reatime air quality data always for you. you can also read our blogs.</p>
      <motion.div initial={{opacity: 0, y: 20}} animate={{opacity:1 , y:0}} transition={{duration: 0.8, delay:0.2}}>
        <Button size="lg" className="group">Check air quality <ArrowRight className="ml-2 h-4 w-4 transition-transform group-hover:translate-x-1"/></Button>
      </motion.div>
    </motion.div>
    {/* Right content */}
