...
export default function LocationSearch() {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        ...
        const filtered: CleanAddress[] = (data.addresses || [])
          ...
          .filter((addr: CleanAddress) => addr.city || addr.state || addr.country)
          .filter((addr: CleanAddress) => /^[\x00-\x7F]*$/.test(addr.city)) // ASCII only
          .filter((addr: CleanAddress, index: number, self: CleanAddress[]) =>
            index ===
            self.findIndex(
              (a) =>
  ...
  const handleSelect = (address: CleanAddress) => {
    ...
    if (cityName) {
      const slug = encodeURIComponent(cityName.toLowerCase().replace(/\s+/g, '-'));
  ...
  return (
    <div ref={wrapperRef} className="relative w-full max-w-md mx-auto">
      <Input value={query} placeholder='Search Location..'
